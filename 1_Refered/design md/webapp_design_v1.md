# Design System — imToken Webapp V1
...
This section keeps the V1 structure and styling inventory, while adding a semantic contract layer for mature component families.

In V1, each mature component should be read from two angles at the same time:

- visual styling: how the component looks, spaces, and binds to tokens
- semantic contract: what the component is for, which public choices it exposes, and which usage boundaries define the family

The semantic layer is intentionally lighter than a full code contract. It exists to make canonical Figma components easier to govern, easier to rename correctly, and easier to keep aligned with system documentation.
...
**Purpose**
- Button is the primary trigger for direct user actions, including confirmation, navigation, and contextual utility tasks.

**Public semantic axes**
- `emphasis`: primary | secondary | outline | ghost | foreground | destructive | link
- `size`: sm | default | lg
- `state`: default | disabled
- `icon`: none | leading | trailing | both, when the canonical family is expanded

**Semantic notes**
- The base family name should remain `Button`; visual differences should be expressed through semantic axes rather than separate component names.
- The current Figma variant label `Hierarchy` should be treated as a visual-era label. The preferred public semantic label is `Emphasis`.
- `primary` is reserved for the dominant action in a local action group.
- `destructive` is reserved for actions that remove, revoke, or irreversibly change user value.
- `link` is a low-surface action, not a substitute for the main CTA.

**Usage rules**
- Keep button labels verb-first and specific.
- Avoid placing two primary buttons side by side in the same decision area.
- Do not create a new button family when the difference is already expressible by `emphasis`, `size`, or `state`.
...
**Purpose**
- IconButton provides compact action access when the icon is already recognizable or when nearby context supplies the missing label.

**Public semantic axes**
- `emphasis`: ghost | muted | foreground
- `size`: sm | md | lg
- `state`: default | hover | pressed | disabled

**Semantic notes**
- IconButton is not a visual replacement for Button. It is a compact action primitive with reduced textual affordance.
- The family should only be used when icon meaning is already familiar or reinforced by nearby copy, layout, or tooltip behavior.
- Badge, status mark, or count overlays should be treated as optional sub-elements, not separate families.

**Usage rules**
- Prefer Button when the action label must carry the meaning.
- Use IconButton in nav bars, toolbars, close actions, refresh actions, and compact contextual affordances.
- Do not rely on icon-only affordance for ambiguous financial actions.
...
**Purpose**
- ActionButton and ActionBar package a group of short, high-frequency actions into a repeatable local action cluster.
...
**Semantic notes**
- This family is a composition pattern built on top of button semantics, not a separate logic model for actions.
- The action bar defines local action hierarchy and density for dashboard or token-operation surfaces.

**Usage rules**
- Use when multiple peer actions must remain visible together.
- Keep one dominant action at most within the local group.
- Do not use the action bar pattern when actions require long explanatory labels or multistep confirmation.
...
**Purpose**
- Card groups related information into a single bounded surface with moderate emphasis and clear internal hierarchy.

**Public semantic axes**
- `tone`: default | elevated | branded | status
- `density`: regular | compact
- `state`: default | hover | selected

**Semantic notes**
- Card is a grouping surface, not a decorative wrapper.
- The card family should preserve slot-based composition rather than forcing one rigid content order.
- Elevation, branded treatment, or status treatment should remain semantic tone choices rather than turning into separate card families.

**Usage rules**
- Use Card when information benefits from a visible surface boundary and reusable slot structure.
- Do not wrap every section in a card if the only goal is to add visual chrome.
- Keep title, body, action, and footer responsibilities distinct when slots are present.
...
**Purpose**
- Input captures editable single-line user data in forms, search flows, address entry, and token-operation steps.

**Public semantic axes**
- `state`: default | focus | disabled | error | success
- `density`: default | compact, when a compact form pattern is introduced
- `content`: empty | filled | placeholder

**Semantic notes**
- Input is a data-entry control first and should remain semantically neutral until validation or status changes are applied.
- Validation appearance belongs to the input state model, not to separate visual-only field variants.
- Placeholder text should support entry, not replace labels.

**Usage rules**
- Use Input for editable user-entered values, not read-only summaries.
- Keep labels, helper text, and validation messaging outside the placeholder.
- Do not rely on focus styling alone to communicate errors or success.
...
**Purpose**
- TokenList presents scannable, repeatable token entities for selection, comparison, and navigation across wallet, send, and portfolio workflows.
...
**Semantic notes**
- TokenList is a collection pattern, not just a styled control. The row anatomy should stay consistent wherever token entities are scanned or selected.
- The family should preserve clear separation between token identity, token amount, and token value regions.
- Selection behavior should be explicit in the contract instead of being implied only by local screen logic.

**Usage rules**
- Use TokenList for repeated token inventory and selection workflows.
- Keep row height, icon position, and text rhythm stable across screens so tokens remain immediately scannable.
- Do not fork the row structure unless the underlying token meaning changes materially.
